...
from selenium.webdriver.support.ui import Select  # 选择下拉列表
...
class TrainSpider(object):
    ...
    def confirm(self):
        ...
        for seat_type in seat_types:
            ...
            except NoSuchElementException:  # 属性不存在异常
                continue
            else:
                break
        WebDriverWait(driver, 1000).until(
        ...
        while submit_btn:
            try:
                submit_btn.click()
                submit_btn = driver.find_element_by_id('qr_submit_id')
            except ElementNotVisibleException:  # 元素找不到异常
                break
...
def start():  # O 表示的是二等座,M 代表是一等座
    spider = TrainSpider('长春', '北京', '2021-04-1', {'G938': ['O', 'M']}, ['杨淑娟'])
    spider.run()
